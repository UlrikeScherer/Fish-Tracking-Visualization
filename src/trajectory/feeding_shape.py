...
import pandas as pd
import numpy as np
from src.config import BACK, FRONT
from src.utils.feeding_maze_config import MAZE, read_maze_data_from_json
...
class FeedingEllipse(FeedingShape):
    ...
    def contains(self, data_points, fish_key, day=None):
        ...
        in_ellipse = ((data_points["xpx"] - ori_x)**2/a**2 + (data_points["ypx"] - ori_y)**2/b**2) <= 1
        return data_points[in_ellipse], np.array([(ori_x+a*np.cos(alpha), ori_y+b*np.sin(alpha)) for alpha in np.linspace(0, 2*np.pi, 100)])
...
def get_feeding_patches():
    ...
    return dict(zip(
        map(lambda stuff:"_".join(map(str,stuff)), patches[["camera_id","front_or_back"]].to_numpy()), 
