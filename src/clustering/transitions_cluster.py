from graph_tool.draw import Graph, graph_draw
import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
from src.config import CAM_POS, DAY
from src.clustering.clustering import get_results_filepath
...
def transition_rates_over_all(fish_keys, clusters, traces_all, trace_size):
    tra_r = transition_rates_for_fish(
    ...
    **kwargs
    ...
    edges = [
        (
            t.index[i],
            t.columns[j],
            t_np[i, j] * eweight_scale,
            max(t_np[i, j] * 3 * eweight_scale, eweight_scale / 2),
            "%.2f" % t_np[i, j],
        )
        for (i, j) in np.transpose(t_np.nonzero())
    ]
...
def plot_transitions_individuality_develpoment(
    ...
    draw_transition_graph(
        ...
        flip_y=True,
        output=get_results_filepath(
            trace_size, "all_transitions_c%s" % t.shape[0], format="png"
        ),
    )

    for fk, X, C, table in iter_fish_subset(
        fish_keys, table, X_embedded, clusters, table
    ):
        t = transition_rates(C)
        fpath = get_results_filepath(
            trace_size,
            "ft_%d_%s" % (n_clusters, fk),
            subfolder="individualty",
            format="png",
        )
        draw_transition_graph(
            t, n_clusters, positions=cluster_means, flip_y=True, output=fpath
        )
        day_before = "0"
        for i, day in enumerate(days[6::7]):
            ...
            if t_d.shape[0] == 3:
                print(fk)
            day_before = day
            weekpath = get_results_filepath(
                trace_size,
                "ft_c%d_%s_week%d" % (n_clusters, fk, i),
                subfolder="development/%s" % fk,
                format="png",
            )
            _ = draw_transition_graph(
                t_d, n_clusters, positions=cluster_means, flip_y=True, output=weekpath
            )
...
def cluster_distribution_over_days(
    table, clusters, n_clusters, days, trace_size, fish_key=None
):
    y = []
    ...
    df.plot(
        kind="bar", stacked=True, colormap="tab10", figsize=(10, 5), rot=45, position=-1
    )
    plt.tight_layout()
