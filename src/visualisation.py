...
import os
import sys
from src.utile import csv_of_the_day, get_position_string, get_time_for_day, BLOCK, ROOT_img
...
def plots_for_tex(camera_list, day_list, marker_char=""):
    i = 0
    N = 2 * len(camera_list) * len(day_list)
    for is_back in [True, False]:
        fo = set_figure(is_back,  marker_char=marker_char)
...
def subplot_trajectory(fig_obj, batch, date, directory, name, time_span="batch: 1,   00:00:00 - 00:30:00", is_back=False):
    (fig, ax, line) = fig_obj
    ...
    #ax.draw_artist(line)
    remove_text = meta_text_for_plot(ax, batch, is_back=is_back)
