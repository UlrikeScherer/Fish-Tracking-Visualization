...
def read_maze_data_from_json(project_path=config.CONFIG_DATA):
    """Reads the maze data from the json-file and returns a dictionary with the
    data for each fish.
    """
    if not os.path.isfile(f"{project_path}/{config.MAZE_FILE}"):
        print("No maze_data.json file found in %s" % project_path)
        return read_maze_data_from_server(config.PATH_RECORDINGS, project_path)
    with open(f"{project_path}/{config.MAZE_FILE}", "r") as f:
...
def read_maze_data_from_server(path_recordings, project_path):
        ...
        with open(f, "r") as fp:
            ...
            for d in jf:
                if "back" in d["comment"].lower():
                    ...
                    print(
                        "missing position in comment: %s for cam: %s"
                        % (d["comment"], cam + " " + day)
                    )
                    print(f)
                    continue
    ...
    with open(f"{project_path}/{config.MAZE_FILE}", "w") as f:
